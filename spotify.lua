...
local function start_liked_at_offset(offset)
    local token = get_spotify_token()
    if not token then return false end
    local payload = string.format('{"context_uri":"spotify:collection:tracks","offset":{"position":%d}}', offset)
...
local function get_playback_status()
    ...
    if not data then current_track_id = nil; return nil end
    local track = data.item
    if not track or type(track) ~= "table" then current_track_id = nil; return nil end
...
local function do_action(action, item, category, context, context_type, context_id, seek_ms, liked_offset)
    local id = item.id
    ...
    if action == "Play" then
        ...
        elseif context == "liked" then
            if liked_offset and liked_order and start_liked_at_offset(liked_offset) then
                -- API call succeeded, playback starts at exact position
            else
...
local function show_actions(item, category, context, context_type, context_id, seek_ms, liked_offset)
    ...
    while true do
        ...
        do_action(sel, item, category, context, context_type, context_id, seek_ms, liked_offset)
    ...
    while true do
        ...
        if category == "artist" then
            ...
            while true do
                ...
                for i, t in ipairs(tracks) do
                    track_entries[#track_entries + 1] = display_track(t)
                end
                while true do
        ...
        elseif category == "track" then
            if context == "liked" then
                show_actions(item, "track", context, nil, nil, nil, idx - 1)
            else
                show_actions(item, "track", context)
            end
        end
